...
export default function SidebarV2({ collapsed, onToggleCollapse }: { collapsed: boolean; onToggleCollapse: () => void }) {
  ...
  const sections: NavSection[] = [
    { label: t('nav.workspace', { defaultValue: '工作區' }), items: [
      ...
      { to: '/companies', label: t('nav.companies', { defaultValue: '公司' }), icon: 'company' },
      { to: '/projects', label: t('nav.projects', { defaultValue: '項目' }), icon: 'target' },
      { to: '/tasks', label: t('nav.tasks', { defaultValue: '任務' }), icon: 'tasks' },
    ]},
    { label: t('nav.records', { defaultValue: '記錄' }), items: [
      { to: '/touchpoints', label: t('nav.touchpoints', { defaultValue: '聯繫記錄' }), icon: 'deal' },
      { to: '/namecards', label: t('nav.namecards', { defaultValue: '名片庫' }), icon: 'folder' },
      { to: '/reports', label: t('nav.reports', { defaultValue: '報表' }), icon: 'report' },
    ]},
    { label: t('nav.organization', { defaultValue: '組織' }), items: [
      { to: '/team', label: t('nav.team', { defaultValue: '團隊' }), icon: 'team' },
      { to: '/ai-apps', label: t('nav.aiApps', { defaultValue: 'AI Apps' }), icon: 'ai' },
      { to: '/notifications', label: t('nav.notifications', { defaultValue: '通知' }), icon: 'bell' },
      { to: '/marketplace', label: t('nav.marketplace', { defaultValue: 'Marketplace' }), icon: 'globe' },
